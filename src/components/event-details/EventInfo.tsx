  ...
  return (
    <div className="space-y-1 mb-8 px-6 pt-6">
      ...
      <div className="text-sm font-semibold text-zinc-600">
        {formattedDate}, {event.start_time ? `${format(parseISO(`2000-01-01T${event.start_time}`), 'HH:mm')}${event.end_time ? ` - ${format(parseISO(`2000-01-01T${event.end_time}`), 'HH:mm')}` : ''}` : 'Time not set'} / {event.pax} Guests / Celebration / <span className="font-bold">{venueNames}</span>
      </div>
    </div>
  );
};
