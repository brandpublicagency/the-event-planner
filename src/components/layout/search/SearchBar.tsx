...
export const SearchBar = () => {
  ...
  return <div className="relative w-64">
      ...
      <Input placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={() => setIsFocused(true)} onBlur={() => {
      // Delay to allow click on results
      setTimeout(() => setIsFocused(false), 200);
    }} className="pl-10 pr-8 bg-zinc-50 border-zinc-200 focus-visible:ring-0 rounded-md" />
      
      {searchQuery && <button className="absolute right-3 top-2.5" onClick={clearSearch}>
          <X className="h-4 w-4 text-muted-foreground" />
        </button>}
      
      {/* Show search results if focused and has query */}
      {isFocused && searchQuery && searchQuery.length >= 2 && <SearchResults isLoading={isLoading} searchResults={filteredResults} />}
