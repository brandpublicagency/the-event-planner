...
const CategoryHeader: React.FC<CategoryHeaderProps> = ({
  ...
  return <div className="flex items-center justify-between mb-2 bg-gray-50 rounded-md px-0 py-0 mx-[3px] my-[4px]">
      ...
      {category !== 'Uncategorized' && <div className="flex items-center space-x-1">
          <button onClick={() => onEditCategory(category)} className="p-1 rounded-md hover:bg-gray-100 transition-colors" title={`Edit ${category} category`}>
            <Edit className="h-3.5 w-3.5 text-gray-500" />
            <span className="sr-only">Edit category</span>
          </button>
          <button onClick={() => onDeleteCategory(category)} className="p-1 rounded-md hover:bg-gray-100 transition-colors" title={`Delete ${category} category`}>
